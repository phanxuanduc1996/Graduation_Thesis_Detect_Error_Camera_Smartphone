...
session = tf.Session()
x = tf.placeholder(tf.float32, shape=[None, 180, 240, num_channels], name='x')
...
def conv_net(x, layer):
    weight = {
        ...
        'W_fc': tf.Variable(tf.truncated_normal([45*60*64, 1024], stddev=0.05)),
    ...
    fc = tf.reshape(conv2, [-1, 45*60*64])
...
def train(num_iteration):
    ...
    for i in range(total_iterations,
        ...
        if i % int(data.train.num_examples / batch_size) == 0:
            ...
            saver.save(session, 'save/Flare_2')
    total_iterations += num_iteration
